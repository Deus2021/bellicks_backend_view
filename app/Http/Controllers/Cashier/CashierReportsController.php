...
class ReportsController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if (!is_null($request->branch_data) && !is_null($request->date_data) && !is_null($request->loans_data)) {
         ...
         */
        if (!is_null($request->loans_data) && !is_null($request->branch_data)) {
            ...
            if ($payment_status == 0) {
                ...
                foreach ($loans as $loan) {
                    ...
                    $loan_info = [
                        ...
                        'paid_amount' => $repayments->sum('repayment_amount'),
                ...
                foreach ($filter as $fil) {
                    ...
                    if ($check->count() > 0) {
                        ...
                        $loan_info = [
                            ...
                            'paid_amount' => $check->sum('repayment_amount'),
                    ...
                    foreach ($payments as $payment) {
                        ...
                        if ($payment->total_repayment_amount % $expected_of_day == 0) {
                            ...
                            if ($date_covered_by_paid_amount > Carbon::now()) {
                                ...
                                'customer_phone' => $loan->customersLoansRelation->customer_phone,
                                'loan_amount' => $loan->loan_amount + $loan->rate_amount,
                                'paid_amount' => $payment->total_repayment_amount,
                                'outstanding_balance' => ($loan->loan_amount + $loan->rate_amount) - $payment->total_repayment_amount,
                                'number_of_days_paid' => $days_covered_by_paid_amount,
                                'paid_up_to' => $date_covered_by_paid_amount->format('Y-m-d'),
                                ];
                                array_push($paid_more, $customer_data);
                                ...
                                array_push($paid_less, $customer_data);
                            }
                        } else {
